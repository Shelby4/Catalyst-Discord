...
module.exports = {
  ...
  data: {
    description: 'Send rules to the specified channel',
  ...
  run: async ({ client, interaction }) => {
    ...
    const pathToRulesJSON = '../../../config/rules.json';
    delete require.cache[require.resolve(pathToRulesJSON)];
    const rules = require(pathToRulesJSON);
    const ruleArr = Object.entries(rules).map(([emoji, rule]) => `> ${emoji} ${rule}`);
    const ruleStr = ruleArr.join('\n> \n');

    // Building our rule embed
    const embedData = {
      color: colorResolver(),
      author: {
        name: `${guild.name} | Rules`,
